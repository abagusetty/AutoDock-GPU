...
/* DPCT_ORIG 	int   run_id = blockIdx.x / cData.dockpars.num_of_lsentities;*/
...
/* DPCT_ORIG 	__shared__ float best_energy;*/

/* DPCT_ORIG 	__shared__ float sFloatAccumulator;*/

/* DPCT_ORIG 	volatile __shared__ float sFloatBuff[6 * MAX_NUM_OF_ATOMS + 5
...
#else
/* DPCT_ORIG 	int3* cartesian_gradient = (int3*)(calc_coords +
 * MAX_NUM_OF_ATOMS);*/
        sycl::int3 *cartesian_gradient =
            (sycl::int3 *)(calc_coords + MAX_NUM_OF_ATOMS);
#endif
...
/* DPCT_ORIG 	for (int i = threadIdx.x ; i < cData.dockpars.num_of_genes; i +=
 * blockDim.x)*/
...
#endif
/* DPCT_ORIG 		float beta1p = 1.0f -
 * pow(cData.dockpars.adam_beta1, 1.0f + iteration_cnt);*/
                float beta1p =
                    1.0f - sycl::pow<double>(cData.dockpars.adam_beta1,
                                             1.0f + iteration_cnt);
/* DPCT_ORIG 		float beta2p = 1.0f -
 * pow(cData.dockpars.adam_beta2, 1.0f + iteration_cnt);*/
                float beta2p =
                    1.0f - sycl::pow<double>(cData.dockpars.adam_beta2,
                                             1.0f + iteration_cnt);

/* DPCT_ORIG 		for(int i = threadIdx.x;*/
			...
			// Applying update
/* DPCT_ORIG 			genotype[i] -= mp / (sqrt(vp) +
 * cData.dockpars.adam_epsilon);*/
...
/* DPCT_ORIG 			__syncthreads(); */
                        /*
                        DPCT1065:61: Consider replacing sycl::nd_item::barrier()
                        with
                        sycl::nd_item::barrier(sycl::access::fence_space::local_space)
...
/* DPCT_ORIG 	for (uint32_t gene_counter = threadIdx.x+3;*/
...
/* DPCT_ORIG 	for (uint gene_counter = threadIdx.x;*/
...
{
/* DPCT_ORIG 	gpu_gradient_minAdam_kernel<<<blocks,
 * threads>>>(pMem_conformations_next, pMem_energies_next);*/
        ...
        */
        dpct::get_default_queue().submit([&](sycl::handler &cgh) {
                ...
                /*
                DPCT1101:190: '6 * MAX_NUM_OF_ATOMS + 5 *
                ...
                */
                sycl::local_accessor<volatile float, 1> sFloatBuff_acc_ct1(sycl::range<
                                                                               1>(
                                                                               1851 /*6 * MAX_NUM_OF_ATOMS + 5 * ACTUAL_GENOTYPE_LENGTH*/),
                                                                           cgh);
                    ...
                    [=](sycl::nd_item<3> item_ct1)
                        [[intel::reqd_sub_group_size(32)]] {
                                gpu_gradient_minAdam_kernel(
                                    ...
                                    item_ct1, *cData_ptr_ct1, entity_id_acc_ct1,
                                    best_energy_acc_ct1,
        ...
        /*
        DPCT1001:189: The statement could not be removed.
        */
