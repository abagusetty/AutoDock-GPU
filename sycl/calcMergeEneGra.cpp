...
SYCL_EXTERNAL void gpu_calc_energrad(float *genotype, float &global_energy,
                                     ...
                                     sycl::nd_item<1>& item, GpuData cData)
{
  float energy = 0.0f;
#if defined (DEBUG_ENERGY_KERNEL)
  interE = 0.0f;
  ...
  for (uint32_t atom_id = item.get_local_id(0);
       ...
       atom_id += item.get_local_range(0))
  {
  ...
  for (uint32_t gene_cnt = item.get_local_id(2);
       gene_cnt < cData.dockpars.num_of_genes;
       gene_cnt += item.get_local_range(2))
  {
  ...
  // ================================================
  for (uint32_t rotation_counter = item.get_local_id(0);
       rotation_counter < cData.dockpars.rotbondlist_length;
       rotation_counter += item.get_local_range(0))
  {
    ...
    if ((rotation_list_element & RLIST_DUMMY_MASK) == 0) // If not dummy rotation
    {
      ...
      sycl::float4 rotation_unitvec{0};
      sycl::float4 rotation_movingvec{0};
...
/*		x10 = cube [idx_100] - cube [idx_000]; // z = 0
  ...
  if (item.get_local_id(0)==0) for (uint32_t contributor_counter = 0; contributor_counter < cData.dockpars.num_of_intraE_contributors; contributor_counter+= 1) {
#else
      for (uint32_t contributor_counter = item.get_local_id(0);
           contributor_counter < cData.dockpars.num_of_intraE_contributors;
           contributor_counter += item.get_local_range(0)) {
#endif
...
/*			float exp_el_DIEL_K = exp_el + DIEL_K;
...
#ifdef FLOAT_GRADIENTS
        relaxed_atomic_ref<float>(&gradient[atom1_id].x()).fetch_sub(priv_intra_gradient_x);
        relaxed_atomic_ref<float>(&gradient[atom1_id].y()).fetch_sub(priv_intra_gradient_y);
        relaxed_atomic_ref<float>(&gradient[atom1_id].z()).fetch_sub(priv_intra_gradient_z);        

        relaxed_atomic_ref<float>(&gradient[atom2_id].x()).fetch_add(priv_intra_gradient_x);
        relaxed_atomic_ref<float>(&gradient[atom2_id].y()).fetch_add(priv_intra_gradient_y);
        relaxed_atomic_ref<float>(&gradient[atom2_id].z()).fetch_add(priv_intra_gradient_z);        
#else
        relaxed_atomic_ref<int>(&gradient[atom1_id].x()).fetch_sub(priv_intra_gradient_x);
        relaxed_atomic_ref<int>(&gradient[atom1_id].y()).fetch_sub(priv_intra_gradient_y);
        relaxed_atomic_ref<int>(&gradient[atom1_id].z()).fetch_sub(priv_intra_gradient_z);        

        relaxed_atomic_ref<int>(&gradient[atom2_id].x()).fetch_add(priv_intra_gradient_x);
        relaxed_atomic_ref<int>(&gradient[atom2_id].y()).fetch_add(priv_intra_gradient_y);
        relaxed_atomic_ref<int>(&gradient[atom2_id].z()).fetch_add(priv_intra_gradient_z);        
#endif
      ...
      sycl::float4 torque_rot{0.0};
      float gx = 0.0f;
      float gy = 0.0f;
      float gz = 0.0f;
      // overall rotation is only for the moving ligand
      for (uint32_t atom_cnt = item.get_local_id(0);
           atom_cnt < cData.dockpars.true_ligand_atoms;
           atom_cnt += item.get_local_range(0)) {
        sycl::float3 r;
 ...
 * {*/
      if ((item.get_local_id(2) == 0) && (cData.dockpars.true_ligand_atoms)) {
 ...
 * cData.dockpars.num_of_atoms; idx += blockDim.x) {*/
      for (uint32_t idx = item.get_local_id(0);
        ...
        /*
          ...
          issues for the generated code.
        */
        float l = sycl::native::recip(sycl::fast_length(rotation_unitvec));
        rotation_unitvec *= l;
        ...
        // Re-using "gradient_inter_*" for total gradient (inter+intra)
#ifdef FLOAT_GRADIENTS
        atom_force = gradient[lig_atom_id];
...
#endif
        torque_tor = sycl::cross(r, atom_force);
 ...
 * DEG_TO_RAD)))); ;*/
        dpct::atomic_fetch_addI32(
          ...
          sycl::rint(sycl::fmin( (float)MAXTERM, sycl::fmax(-MAXTERM, TERMSCALE * torque_on_axis * DEG_TO_RAD))) );
        /*(M_PI / 180.0f)*/;
#endif
      }
      item.barrier();
