...
private:
    ...
    for(auto& dev: sycl_all_devs) {
      if(dev.get_info<sycl::info::device::partition_max_sub_devices>() > 0) {
        ...
        for(auto& tile: subDevicesDomainNuma) {
          ...
          _queues.push_back(std::make_pair(std::make_shared<device_ext>(tile),
                                           std::make_shared<sycl::context>(tile, sycl::property_list{sycl::property::queue::in_order{}})));
        }
      }
      else {
        _devs.push_back(
        ...
        _queues.push_back(
          std::make_pair(std::make_shared<device_ext>(dev), std::make_shared<sycl::queue>(dev, sycl::property_list{sycl::property::queue::in_order{}})));
      }
    }
  }
  ...
  std::vector<std::pair<std::shared_ptr<device_ext>, std::shared_ptr<sycl::queue>>>   _queues;
...
/// Util function to get the current sycl::queue* for current-device ID
